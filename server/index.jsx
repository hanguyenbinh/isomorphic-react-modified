...
function * getQuestion (oid) {
    ...
    if (useLiveData) {
        ...
        var oids = {}
        oids[oid] = 0;
        data = yield get(question,
                {
                    gzip:true,
                    json:true, 
                    body:{
                        oids:oids,
                        sections:["basic_info","summary"]
                    },
                });
    } else {
        /**
        ...
        const question = questions.items.find(_question=>_question.oid == oid);
        /**
         * Create a mock body for the question
         */
        question.body = `Mock question body: ${oid}`;
        data = {items:[question]};
    }
    return data;
}
...
app.get(['/','/questions/:id'], function *(req,res){
    ...
    if (req.params.id) {
         ...
         */
        const response = yield getQuestion(oid);
        if (response.data[oid]){
            const questionDetails = response.data[oid].basic_info;
            initialState.questions = [{...questionDetails,oid}]
        }
        //console.log("fdsfasfasfdsa", response.data)
        ...
        initialState.questions = [...questions.data]
    }
